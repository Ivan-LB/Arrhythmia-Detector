...
# Windows
DATASET_DIRECTORY = 'C:\\Users\\XPG\\Desktop\\DiagnosticoAsistido\\Arrhythmia-Detector\\Data\\Dataset\\Train'
CSV_FILE_PATH = 'C:\\Users\\XPG\\Desktop\\DiagnosticoAsistido\\Arrhythmia-Detector\\Data\\'
# Mac
...
def process_record_thread(record_name, annotation_mapping, rhythm_annotation_mapping):
    ...
    for i, peak in enumerate(peaks):
        ...
        if signal_windowed is not None:
            ...
            # Cálculo de asimetría, curtosis y varianza
            skewness_val = skew(signal_windowed_normalized)
            kurtosis_val = kurtosis(signal_windowed_normalized)
            variance_val = np.var(signal_windowed_normalized)
            ...
            create_dataframe(features, record_name)
...
def create_dataframe(features, record_name):
    file_path = os.path.join(CSV_FILE_PATH, f"Thread{record_name}.csv")
    if not os.path.exists(file_path):
        # Crear un nuevo DataFrame y guardarlo si el archivo no existe
        df = pd.DataFrame([features])
        df.to_csv(file_path, index=False)
    else:
        # Si el archivo ya existe, añadir a él
...
def combine_csv_files():
    all_files = [f for f in os.listdir(CSV_FILE_PATH) if f.startswith('Thread') and f.endswith('.csv')]
    combined_df = pd.concat([pd.read_csv(os.path.join(CSV_FILE_PATH, f)) for f in all_files])
    combined_csv_path = os.path.join(CSV_FILE_PATH, 'combined_ecg_features.csv')
...
def main():
    ...
    # Lista para mantener los threads
    threads = []

    # Crear un thread para cada registro y empezar su ejecución
    for record_name in record_names:
        thread = threading.Thread(target=process_record_thread, args=(record_name,annotation_mapping,rhythm_annotation_mapping))
